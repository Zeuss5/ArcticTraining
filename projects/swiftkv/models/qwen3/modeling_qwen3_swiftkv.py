...
class Qwen3SwiftKVAttention(Qwen3Attention):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Apply QK-Norm
        if swiftkv_key_states is not None and hasattr(self, 'q_norm_swiftkv'):
            # SwiftKV mode - use swiftkv norms
            query_states = self.q_norm_swiftkv(query_states)
            key_states = self.k_norm_swiftkv(key_states)
        elif hasattr(self, 'q_norm'):
            # Regular mode - use standard norms from parent class
...
class Qwen3SwiftKVModel(Qwen3Model):
    ...
    Each layer is a [`Qwen3SwiftKVDecoderLayer`].

    Args:
        config: Qwen3Config
    """

    config_class = Qwen3SwiftKVConfig

    def __init__(self, config: Qwen3SwiftKVConfig):
...
class Qwen3SwiftKVForCausalLM(Qwen3ForCausalLM):

    config_class = Qwen3SwiftKVConfig

    def __init__(self, config):
        super(Qwen3ForCausalLM, self).__init__(config)
        self.model = Qwen3SwiftKVModel(config)
        self.vocab_size = config.vocab_size
